...
facets = mesh.locate_entities_boundary(
    ...
    marker=lambda x: np.isclose(x[0], 0.0) & (x[1]>=0.1) & (x[1]<=0.3),
)
dofs = fem.locate_dofs_topological(V=V, entity_dim=1, entities=facets)
bc = fem.dirichletbc(value=ScalarType(2.0), dofs=dofs, V=V)
...
from test_fenicsx_solver import generate_source_F, plot_source_and_bc, plot_u
F = generate_source_F(layout_list = [(0.8, 0.5, 0.1, 0.2), (0.3, 0.2, 0.1, 0.1)],
                      powers = [20, 7],
                      length = 1, 
                      nx = 32)
length = 1
...
values = F.ravel()                            # shape=(N,)

def source_func(x):
    # x: shape=(2, M)
    # points: shape=(N, 2)，values: shape=(N,)
    # 假设points是规则网格，按行主序排列
    n = int(np.sqrt(points.shape[0]))
    length = 1 # 这里需要你传入物理区域边长
    F = values.reshape((n, n))
    vals = []
    for xi in x.T:
        xx = int(xi[0] / length * (n - 1))
        yy = int(xi[1] / length * (n - 1))
        vals.append(F[yy, xx])
    return np.array(vals)

f = fem.Function(V)
f.interpolate(lambda x: source_func(x))
...
u = ufl.TrialFunction(V)
v = ufl.TestFunction(V)
dx = ufl.Measure("dx", domain=msh)  # 显式定义积分测度
